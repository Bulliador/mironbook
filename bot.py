...
def save_route(admin_message_id, user_chat_id):
    with db_connect() as con:
        con.execute("""
            ...
            VALUES (?, ?, ?)
        """, (admin_message_id, user_chat_id, now_utc_iso()))
        con.commit()
...
def save_start_event(user_id, chat_id, full_name, username):
    with db_connect() as con:
        con.execute("""
            INSERT INTO start_events
            (user_id, chat_id, full_name, username, started_at)
            VALUES (?, ?, ?, ?, ?)
        """, (user_id, chat_id, full_name, username, now_utc_iso()))
        con.commit()
...
def get_recent_starts(limit=20):
    with db_connect() as con:
        rows = con.execute("""
            SELECT user_id, full_name, username, started_at
            FROM start_events
            ORDER BY id DESC
            LIMIT ?
        """, (limit,)).fetchall()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id == ADMIN_ID:
        await update.effective_message.reply_text(
            ...
            "Когда кто-то напишет боту или нажмёт /start, ты увидишь это здесь.\n"
...
async def starts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id, full_name, username, started_at in rows:
        dt_utc = datetime.fromisoformat(started_at)
...
async def admin_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg.reply_to_message:
        await msg.reply_text(
            "Чтобы отправить ответ пользователю, нажми Reply на его сообщение или карточку."
        )
        return

    replied = msg.reply_to_message
    user_chat_id = get_user_chat_id(replied.message_id)

    if not user_chat_id:
        user_chat_id = extract_user_id_from_admin_card(replied)
